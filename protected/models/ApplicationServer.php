 ...
 * @property string $url
 * @property string $deploy_mode_config
 */
class ApplicationServer extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, url, deploy_mode_config', 'required'),
			array('name, update_mode', 'length', 'max'=>32),
			array('url', 'length', 'max'=>128),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, update_mode, url, deploy_mode_config', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'url' => 'Url',
			'deploy_mode_config' => 'Deploy Mode Config',
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('deploy_mode_config',$this->deploy_mode_config,true);
